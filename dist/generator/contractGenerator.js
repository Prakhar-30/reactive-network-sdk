    ...
    _validateConfig(config) {
        ...
        if (config.eventSignature && config.callbackFunction) {
            ...
            eventConfigurations.push({
                ...
                callbackArgumentMapping: this._processCallbackArguments(config.callbackArgumentMapping || [], eventSignature, callbackFunction)
            });
        }
            ...
            eventConfigurations = config.events.map(eventConfig => {
                const eventSignature = (0, signatureValidator_1.validateAndFormatEventSignature)(eventConfig.eventSignature, 'eventSignature');
                const eventTopic0 = `0x${(0, signatureValidator_1.generateTopic0)(eventSignature).substring(2)}`;
                const callbackFunction = (0, signatureValidator_1.validateAndFormatFunctionSignature)(eventConfig.callbackFunction, 'callbackFunction');
                return {
                    ...
                    callbackArgumentMapping: this._processCallbackArguments(eventConfig.callbackArgumentMapping || [], eventSignature, callbackFunction)
                };
            });
        }
        else {
